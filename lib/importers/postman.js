...
Postman.prototype._mapRequestHeaders = function(data) {
  ...
  for (var j in headers) {
    ...
    headerObj['properties'][header.key] = {
      type: 'string',
      default: transformVariableFormat(header.value)
    };
  }
  return headerObj;
};
...
Postman.prototype._mapEndpoint = function(pmr) {
  ...
  endpoint.Id = pmr.id;
  urlParts = pmr.request.url.raw.split('?');
  endpoint.QueryString = urlParts.length ? parseQuery(urlParts[1]) : '';
  endpoint.Method = pmr.request.method;
  ...
  //parse headers
  endpoint.Headers = this._mapRequestHeaders(pmr.request.header);
  //TODO map Body
  endpoint.Consumes = mapConsumes(pmr.request.body);

  if (!_.isEmpty(pmr.request.body)) {
    // is it parsable?
    var data = pmr.request.body;
    ...
    endpoint.Body = body;
  } else if (!_.isEmpty(pmr.request.body)) {
    endpoint.Body = this._mapRequestBody(pmr.request.body);
  }
...
Postman.prototype._mapSavedEntry = function(pmr) {
  ...
  var urlParts = pmr.request.url.raw.split('?');
  savedEntry.Id = pmr.id;
  savedEntry.QueryString = urlParts.length ? parseQuery(urlParts[1]) : '';
  savedEntry.Path = transformVariableFormat(urlParts[0]);
  savedEntry.Method = pmr.request.method;
  savedEntry.PathParams = this._mapURIParams(pmr.pathVariables);
  savedEntry.Headers = this._mapRequestHeaders(pmr.request.header);
  savedEntry.Consumes = mapConsumes(pmr.request.body);
  if (savedEntry.Method.toLowerCase() !== 'get' &&
      savedEntry.Method.toLowerCase() !== 'head') {
    savedEntry.Body = this._mapRequestBody(pmr.request.body);
  }
...
Postman.prototype._import = function() {
  ...
  this.project = new Project(this.data.info.name);
  this.project.Description = this.data.info.description;

  var requests = this.data.item || [];
